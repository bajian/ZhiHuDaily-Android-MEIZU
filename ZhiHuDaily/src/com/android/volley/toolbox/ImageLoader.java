...
public class ImageLoader {
	...
	public ImageContainer get(String requestUrl, final ImageListener listener,
			Context context) {
		return get(requestUrl, listener, 0, 0, context);
	}
	...
	public ImageContainer get(String requestUrl, ImageListener imageListener,
			int maxWidth, int maxHeight, Context context) {
		...
		// 无图模式代码开始
		DiskBasedCache diskBasedCache = new DiskBasedCache(new File(context
				.getCacheDir().getAbsoluteFile() + "/volley"));
		if (!new File(diskBasedCache.getFileForKey(newRequest.getCacheKey())
				.toString()).exists()) {
			// 网络判断
			if (PerferenceHelper.getIsOnlyDownImageInWifi(context)) {
